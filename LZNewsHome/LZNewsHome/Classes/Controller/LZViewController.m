...
//        btn.backgroundColor = LZRandomColor;
    ...
    self.contentScrollView.contentSize = CGSizeMake(count * LZScreenW, 0);
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    // 1.拿到滚动到的视图的下标
    NSInteger i = (NSInteger)(scrollView.contentOffset.x / LZScreenW);
    // 2.获取选中标题按钮
...
{
    // 1.拿到左边按钮下标,一开始为0
    NSInteger leftI = (NSInteger)(scrollView.contentOffset.x / LZScreenW);
    // 获取左边按钮
    ...
    // 4.计算缩放大小,,,,0 ~ 1
    // 4.1计算右边按钮缩放大小
    CGFloat rightScale = scrollView.contentOffset.x / LZScreenW - leftI;
    // 4.1计算左边边按钮缩放大小
...
- (void)setupOneChildViewController:(NSInteger)i
{
    ...
    CGFloat x = i * LZScreenW;
    // 设置vc的view的位置
    vc.view.frame = CGRectMake(x, 0, LZScreenW, self.contentScrollView.bounds.size.height);
...
//    NSLog(@"love--%@", NSStringFromCGAffineTransform(self.selectedButton.transform));
    ...
    // 设置偏移量
    CGFloat offsetX = button.center.x - LZScreenW * 0.5;
    ...
    // 2.判断最大时刻
    CGFloat maxFloat = self.titleScrollView.contentSize.width - LZScreenW;
    if (offsetX > maxFloat) {
        offsetX = maxFloat;
    }
